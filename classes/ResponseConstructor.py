...
'''
A class to represent the construction of a response
to a query.
...
class ResponseConstructor:
    ...
    def compareQuestionToAllKnownQuestions(self, quesiton):
        ...
        for possibleQuestion in self.responses["queries"].keys():
            check = self.similar(possibleQuestion, quesiton)
            if check > highest:
                highest = check
                print(highest)
                found   = self.responses["queries"][possibleQuestion]
        return found
    ...
    def buildResponse(self, responseType, context=None):
        ...
        if responseType == "weather data":
            response = response + self.weather.currentSummary() + " and the current temperature is " + str(
                self.weather.currentTemperature()) + " degrees"
        elif responseType == "weather week":
            response = response + self.weather.dailySummary()
